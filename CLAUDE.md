...
- ~~旧Vercel環境を止める~~ → GitHub Actionsのワークフローは PR #2 のマージで削除済み
  (リポジトリのワークフロー数が0であることを確認)。Vercelプロジェクトも削除済み。
  **app1を消費し続けていた経路はすべて断たれた**
- ~~hotel-bargain-php PR #29 のマージ~~ → マージ済み(スキーマ変更は無いので
  マイグレーション不要)
- ~~Search Consoleにサイトマップを再送信~~ → 送信済み。ただし下記の順序に注意

### 残っているもの

1. **本番デプロイ(`bash deploy/conoha-sync.sh`)** — PR #29 の内容はまだ本番に
   出ていない可能性がある。マージとデプロイは別作業なので注意
2. デプロイ後、`~/hotel-bargain/logs/` に書き込み権限があることを確認
   （ページキャッシュの置き場。無くてもキャッシュが効かないだけでページは出る）
3. **サイトマップの再送信は、デプロイ後にもう一度行う必要があるかもしれない。**
   宿ページ(約280URL)は本番にデプロイされて初めて存在する。デプロイ前に送信した
   場合、Googleが読んだのは旧バージョンの12URLのサイトマップなので、
   デプロイ後に再送信すること
4. app1の復旧を確認（`rakuten_get_area_class()`を単体で叩く）。旧環境を消したので
   時間経過で解除されるはず
